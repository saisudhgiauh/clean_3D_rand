...
# Note: The variable names are kept for consistency, but this is now a direct maximum, not interpolated.
peak_wvls = np.zeros((len(alphas), len(betas)))
max_Rs = np.zeros((len(alphas), len(betas)))

for i in range(avg_refl_coeffs.shape[0]):
    for j in range(avg_refl_coeffs.shape[1]):
        # Get the reflection spectrum for the current (α, β) angle pair
        spectrum = avg_refl_coeffs[i, j, :]
        
        # Find the index of the highest peak in the spectrum
        max_index = np.argmax(spectrum)
        
        # Use the index to get the corresponding wavelength and reflection value
        peak_wvls[j, i] = avg_wvls[max_index]
        max_Rs[j, i] = spectrum[max_index] # This is the same as avg_refl_coeffs[i, j, max_index]
        
#lazy norm
interp_max_Rs = np.divide(np.abs(max_Rs),np.amax(np.abs(max_Rs)))
...
fig = plt.figure()
ax = plt.axes(projection="3d")
ax.plot_surface(aa,bb,peak_wvls,cmap="hot",edgecolor="None")
ax.set_xlabel("α")
ax.set_ylabel("β")
