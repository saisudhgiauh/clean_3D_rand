...
def get_ridge(x,eta_degrees):
    ...
    outputs:
        geom_list (list of GeometricObject classes) -- the MEEP objects that comprise the ridge, to be appended to the simulation geometry list.
    '''
    ...
    if round_centres == True:
        ...
        #now we add the branches by appending additional meep objects to the geom_list list
    ...
    geom_list += [mp.Prism(       #overlap triangular prism
            vertices1,
            height=trunk_width,
            axis=bv,
            material=wing_mat
    ...
    geom_list += [mp.Prism(       #overlap quad prism
            vertices2,
            height=trunk_width,
            axis=bv,
            material=wing_mat
    ...
    if round_top:    
        if overlap != 0:
            ...
            overhang_angle = 0
        geom_list += tf.roundmask_3D(     #round overhang of quad prism
            height=trunk_width,
        ...
        geom_list += tf.roundmask_3D(     #round top of quad prism
            height=trunk_width,
            width=(3/2) * trunk_width,      
    ...
    geom_list += [mp.Block(       #fix bottom of trunk
...
for i in range(len(etas)):
    ...
    actual_geometry += get_ridge(position,etas[i])    
...
end_time = time.time()
print(f"Done, structure generated in {end_time - start_time} seconds")
# %%