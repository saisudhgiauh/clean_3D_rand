...
x_pts = np.linspace(0,n2f_width/2,40)
z_pts = np.linspace(0,n2f_width/2,40)
...
interp_peak_wvls = np.zeros((len(alphas),len(betas)))   #parabolically interpolated maximum power for each pair of reflected angles
interp_max_Rs = np.zeros((len(alphas),len(betas)))      #parabolically interpolated wavelength (corresponding to max power) for each pair of reflected angles.
for i in range(avg_refl_coeffs.shape[0]):
    for j in range(avg_refl_coeffs.shape[1]):
